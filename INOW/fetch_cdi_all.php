...
<?php
    ...
    $SQL = "SELECT student_id, application_id, id FROM submissions WHERE student_id <> '' AND application_id = 62 AND enrollment_id = 17 AND cdi_override = 'N'";
    $rs = $objDB->select($SQL);
    ...
    if(count($rs) > 0)
    {
        if(count($rs) > 0)
        {
            ...
            for($i=0; $i < count($rs); $i++)
            {
                ...
                if(count($rs1) > 0)
                {
                    $arr[] = $rs1[0]['student_id'];
                    $arr1[] = $rs1[0]['stateID'];
                    $submissions[] = $rs[$i]['id'];
                    $applications[] = $rs[$i]['application_id'];
