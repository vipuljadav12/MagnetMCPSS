...
ini_set("error_reporting","1");
    ...
    $SQL = "SELECT stateID, student_id FROM student WHERE grade_fetched = 'N' LIMIT 1";
    $rs = $objDB->select($SQL);

    $arr = $arr1 = array();
    for($i=0; $i < count($rs); $i++)
    {
        $SQL = "SELECT stateID FROM studentgrade WHERE stateID = '".$rs[$i]['stateID']."'";
        ...
        {
            $SQL = "UPDATE student SET grade_fetched = 'Y' WHERE stateID = '".$rs[$i]['stateID']."'";
