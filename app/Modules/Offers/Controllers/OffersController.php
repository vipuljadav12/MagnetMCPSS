...
class OffersController extends Controller
{
    ...
    public function offerChoice($slug)
    {
        ...
        if ((date("Y-m-d H:i:s") > $last_online_date && $submission->submission_status != "Offered and Accepted") || (date("Y-m-d H:i:s") > $last_online_date  && $submission->submission_status == "Offered and Accepted" &&  $submission->contract_status == "Signed")) {
            ...
            if (!empty($submission)) {
                ...
                if ($submission->contract_mode != "Pending" && $submission->first_offer_status != "Pending" && $submission->second_offer_status != "Pending") {
    ...
    public function offerSave(Request $request)
    {
        ...
        $date = DistrictConfiguration::where("enrollment_id", $rsenv->enrollment_id)->where("name", "last_date_online_acceptance")->select("value")->first();
        $last_online_date = date("Y-m-d H:i:s", strtotime($date->value));

        $date = DistrictConfiguration::where("enrollment_id", $rsenv->enrollment_id)->where("name", "last_date_offline_acceptance")->select("value")->first();
        $last_offline_date = date("Y-m-d H:i:s", strtotime($date->value));
    ...
    public function autoDecline()
    {
        ...
        foreach ($rs as $key => $value) {
            ...
            $date_online = DistrictConfiguration::where("name", "last_date_online_acceptance")->where("enrollment_id", $enrollment_id)->select("value")->first();
            $date_wt_online = DistrictConfiguration::where("name", "last_date_waitlist_online_acceptance")->where("enrollment_id", $enrollment_id)->select("value")->first();
            $date_ls_online = DistrictConfiguration::where("name", "last_date_late_submission_online_acceptance")->where("enrollment_id", $enrollment_id)->select("value")->first();

            if (!empty($date_online))
                $last_online_date = date("Y-m-d H:i:s", strtotime($date_online->value));
            else
                $last_online_date = '';

            if (!empty($date_wt_online))
                $last_wt_online_date = date("Y-m-d H:i:s", strtotime($date_wt_online->value));
            else
                $last_wt_online_date = '';

            if (!empty($date_ls_online))
                $last_ls_online_date = date("Y-m-d H:i:s", strtotime($date_ls_online->value));
            else
                $last_ls_online_date = '';
    ...
    public function storeContractConfiguration(Request $request)
    {
        return $request;
        $district_id = Session::get('district_id');
        $enrollment_id = Session::get('enrollment_id');
    ...
    {
        return view('Offers::contract_preview');
    }
}
