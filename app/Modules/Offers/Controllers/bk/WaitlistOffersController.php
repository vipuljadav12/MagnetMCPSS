...
//                        return view("Offers::Waitlist.contract_later", compact("submission", "application_data", "msg"));
                ...
                return view("Offers::Waitlist.index",compact('submission','first_program', 'second_program', "last_online_date", "last_offline_date", "application_data", "approve_program_id", "msg"));
            }
            else
            {
                echo "T";exit;
    ...
    public function offerSave(Request $request)
    {
        ...
        if(isset($request->accept_btn))
        {
            ...
            $data = SubmissionsWaitlistFinalStatus::where("submission_id", $submission_id)->first();
            if(!empty($data))
            {
                $redirectN = true;
                if($data->first_choice_final_status == "Offered" && $data->first_waitlist_for == $program_id)
                {
                    $rs = SubmissionsWaitlistFinalStatus::where("submission_id", $submission_id)->update(["first_offer_status"=>"Accepted", "first_offer_update_at"=>date("Y-m-d H:i:s"), "second_offer_status"=>"NoAction"]);
                ...
                {
                    $rs = SubmissionsWaitlistFinalStatus::where("submission_id", $submission_id)->update(["second_offer_status"=>"Accepted", "second_offer_update_at"=>date("Y-m-d H:i:s"), "first_offer_status"=>"NoAction"]);
                    $program_name = getProgramName($submission->first_waitlist_for);
                    $program_id = $submission->first_waitlist_for;
                    $redirectN = false;
                }
        ...
        elseif(isset($request->decline_btn))
        {
            ...
            $submission = SubmissionsWaitlistFinalStatus::where("submission_id", $submission_id)->join("submissions", "submissions.id", "submissions_waitlist_final_status.submission_id")->select("submissions_waitlist_final_status.*", "submissions.*")->first();
            $tmp = generateShortCode($submission);
            $str = DistrictConfiguration::where("name", "waitlist_offer_declined_screen")->select("value")->first();
            $msg = $str->value;
            $msg = find_replace_string($msg,$tmp);
            ...
            $rs = SubmissionsWaitlistFinalStatus::where("submission_id", $submission_id)->update(["first_offer_status"=>"Declined", "first_offer_update_at"=>date("Y-m-d H:i:s"), "second_offer_status"=>"Declined", "second_offer_update_at"=>date("Y-m-d H:i:s")]);
            $application_data = Application::join("enrollments", "enrollments.id", "application.enrollment_id")->where('application.id', $submission->application_id)->where('application.district_id', Session::get('district_id'))->where("application.status", "Y")->select("application.*", "enrollments.school_year")->first();
        ...
        elseif(isset($request->decline_waitlist))
        {
            ...
            {
                $rs = SubmissionsWaitlistFinalStatus::where("submission_id", $submission_id)->update(["first_offer_status"=>"Declined & Waitlisted", "first_offer_update_at"=>date("Y-m-d H:i:s"), "second_offer_status"=>"Waitlisted"]);
            ...
            {
                $rs = SubmissionsWaitlistFinalStatus::where("submission_id", $submission_id)->update(["second_offer_status"=>"Declined & Waitlisted", "second_offer_update_at"=>date("Y-m-d H:i:s"), "first_offer_status"=>"Waitlisted"]);
                $program_name = getProgramName($submission->second_waitlist_for);
            }
