...
use App\Modules\Submissions\Models\{LateSubmissionFinalStatus, SubmissionsFinalStatus};
...
class LateSubmissionEditCommunicationController extends Controller
{
    ...
    {
        $display_outcome =  app('App\Modules\LateSubmission\Controllers\LateSubmissionController')->checkWailistOpen();

        $statusArr = $this->statusArr;
        $dbStatusArr = $this->dbStatusArr;

        $data = LateSubmissionEditCommunicationLog::where("status", $dbStatusArr[$status])->where("district_id", Session::get("district_id"))->orderByDesc("created_at")->get();
        ...
        $data = LateSubmissionEditCommunication::where('district_id', $district_id)->where('status', $dbStatusArr[$status])->first();
        return view('LateSubmission::edit_communication_index', compact('data', 'statusArr', 'status', 'dbStatusArr', 'letter_download_data', 'mail_download_data', "display_outcome"));
    }
    ...
    public function storeLetter(Request $request)
    {
        ...
        $rs = LateSubmissionEditCommunication::updateOrCreate(["status" => $data['status']], $data);
        ...
        return redirect('admin/LateSubmission/EditCommunication/' . $req['redirect_status']);
    }
    ...
    public function generate_letter_now($status, $preview = false)
    {
        ...
        $rs = LateSubmissionFinalStatus::where("enrollment_id", Session::get("enrollment_id"))->where("version", $version)->count();
        if ($rs == 0) {
            $version = $version - 1;
            $rs = LateSubmissionFinalStatus::where("enrollment_id", Session::get("enrollment_id"))->where("version", $version)->count();
        }
        ...
        $district_id = Session::get("district_id");
        $cdata = LateSubmissionEditCommunication::where("status", $status)->first();
        ...
        $rs = DistrictConfiguration::where("enrollment_id", Session::get("enrollment_id"))->where("name", "last_date_late_submission_online_acceptance")->select("value")->first();
        $last_date_online_acceptance = getDateTimeFormat($rs->value);

        $rs = DistrictConfiguration::where("enrollment_id", Session::get("enrollment_id"))->where("name", "last_date_late_submission_offline_acceptance")->select("value")->first();
    ...
    public function send_email_now($status, $preview = false)
    {
        ...
        if ($preview == true) {
            ...
            return view("emails.preview_late_submission_index", compact('data', 'status', 'type'));
        } else {
            $countMail = 0;
            foreach ($submissions as $key => $value) {
                $application_data1 = Application::join("enrollments", "enrollments.id", "application.enrollment_id")->where('application.district_id', Session::get('district_id'))->where("application.enrollment_id", Session::get("enrollment_id"))->where("application.status", "Y")->where("application.id", $value->application_id)->select("application.*", "enrollments.school_year")->first();
                ...
                if (($value->first_choice_final_status == $status && $status == "Offered") || ($value->first_choice_final_status == "Offered" && $status == "Offered and Waitlisted") || ($value->first_choice_final_status == $status)) {
                    ...
                    $tmp['module'] = "Edit Communication - Late Submission";
                    ...
                    if (config('variables.environment') == 'production')
                        $countMail = 0;
                    if ($countMail == 0) {
                        //  $tmp['email'] = "chasitie@theleanleap.com";
                        sendMail($tmp, true);
                    }
                    $countMail++;
                }
